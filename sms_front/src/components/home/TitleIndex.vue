<template>
  <div class="title-index">
    <div class="month">
      <ul class="tabs">
        <li v-for="(item,index) in tabsParam" :key="index" :class="{ active: index === nowIndex }"
            class="li-tab" @click="toggleTabs(index)">
          {{ item }}
          <span></span>
        </li>
      </ul>
    ...
    <div class="date-picker">
      <el-date-picker v-model="value" end-placeholder="结束日期" format="yyyy-MM-dd" range-separator="-"
                      start-placeholder="开始日期" type="daterange" value-format="yyyy-MM-dd HH:MM:SS"
                      @change="getArrDate"></el-date-picker>
    </div>
    <div class="get-time">
      <p>数据获取时间：{{ newDate }}</p>
    </div>
...
import {formatDate, get1stAndToday} from '@/utils/formValidate'
