<template>
  <div class="upload-btn" style="display:inline-block;">
    <input ref="upload" hidden style="display:none;" type="file" @change="change"/>
    <el-button :disabled="disabled" type="primary" v-bind="$attrs" @click="selectFile" v-on="$listeners">
      <slot></slot>
      <div v-show="show" class="progress-fixed">
        <el-progress :percentage="percentage" :stroke-width="18" :text-inside="true" status="success"></el-progress>
      </div>
    </el-button>
    <ToastDialog :dialogShow="dialogShow" title='' @closeDialog="closeDialog">{{ commonTitle }}</ToastDialog>
  </div>
</template>
