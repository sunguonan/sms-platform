<template>
  <div class="WholeSMSDialog">
    <el-dialog :before-close="closeDialog" :close-on-click-modal="false" :show-close="true" :title="title"
               :visible.sync="dialogShow" top="22vh" width="660px">
      <m-form ref="mform" :columns="formColumns" :formData="formData">
        ...
        <div v-if="formData.value === 0" slot="fileUpdate">
          <UploadBtn :disabled="fileName.length >= 1" :fileSuffix="fileSuffixXls" icon="iconfont icon-wenjian"
                     maxSize="10" style="margin-left: 15px;vertical-align: middle;" type="primary" updateType="营销"
                     @success="wholeSMSSuccess">选择文件
          </UploadBtn>
          ...
          <div style="margin-top: 6px;margin-left: 15px;">
            <p v-for="(item, index) in fileName" :key="index" style="color: #20232a;">
              ...
              <el-button v-if="item.errormsgs.length > 0" style="margin-left: 20px;color: #0887e0;" type="text"
      ...
      <div slot="footer" style="text-align:center;">
        ...
        <el-button style="margin-bottom:20px;padding: 10px 20px;" type="primary" @click="addOrUpdateSign">确认
        </el-button>
      </div>
    </el-dialog>
        ...
        <div style="border-radius: 10px;">
          ...
          <p style="text-align:center;margin-bottom: 17px;">
            <span style="color: #333333;font-size: 16px;font-weight: 500;">成功导入</span> <span
              style="color: #0887e0;font-weight: 700;font-size: 16px;">{data.count - data.fail}</span>&nbsp;&nbsp;
            ...
            change: () => {
              this.formData.paramFieldsList.forEach((obj: any) => {
                if (obj.objValue && this.formData.content) {
                  ...
                  this.postTemplateBuild({
                    code: this.formData.template,
                    content: JSON.stringify(this.formData.paramFieldsList.map((obj: any) => ({[obj.objKey]: obj.objValue})))
                  })
                  // this.$forceUpdate()
                }
              })
          ...
          render:
              this.formData.list.length !== 1 && index !== 0 ? (
                  <el-button type="text" onClick={() => {
                    ...
                    <el-tooltip class="item" effect="dark" content={`点击删除第${index + 1}收信人手机号`}
                                placement="top">
