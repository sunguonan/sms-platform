<template>
  <div class="passageway-manage">
    <div class="search">
      <m-form :columns="formColumns" :formData="formData"/>
    </div>
    <div class="">
      <el-button style="margin-left:0px;" type="primary" @click="addPassageWay('add')">
    ...
    <div v-if="cardLiStData.length > 0" class="data-list">
      <!-- 数据可拖动部分 -->
      <draggable v-model="cardLiStData" :move="checkMove" class="draggable-list" filter=".undraggable"
                 style="width: 100%; cursor: move;" @end="getNewCardList">
        <el-card v-for="(item, index) of cardLiStData" :key="index" :class="[ item.canEdit ? 'draggable' : 'undraggable', item.isEnable === 0 ? 'active' : '' ]"
                 class="box-card">
          <div class="card">
            <div class="card-title">
              <span>{{ item.name }}</span>
              <el-tooltip class="item" content="点击切换通道状态" effect="dark" placement="top">
                <!-- isActive isEnable -->
                <el-switch v-model="item.isActive" :disabled="item.isActive === 0"
                           active-color="rgba(11,178,144,1)" style="margin-left:15px;width: 32px;height:16px;"
                           @change="getSwitchValue(item)"></el-switch>
              </el-tooltip>
              <el-tooltip class="item" content="点击修改通道" effect="dark" placement="top">
                <img alt="" class="img-edit" src="@/assets/messageService/edit.png"
                     @click="addPassageWay('edit', item)"/>
              </el-tooltip>
              <el-tooltip class="item" content="点击设置" effect="dark" placement="top">
                <img alt="" class="img-shezhi" src="@/assets/messageService/shezhi.png" @click="goChannel(item)"/>
              </el-tooltip>
              <el-tooltip class="item" content="点击删除当前通道" effect="dark" placement="top">
                <img alt="" class="img-delete" src="@/assets/messageService/delete.png" @click="deletePassWay(item)"/>
              </el-tooltip>
            </div>
            ...
            <!-- 新增图片放置 -->
            <div class="level-img">
              <img v-if="index === 0" alt="" class="image-card" src="@/assets/messageService/huangguan.png"/>
              <img v-if="index !== 0 && item.isActive === 0" alt=""
                   class="image-card" src="@/assets/messageService/zuanshi1.png"/>
              <img v-if="index !== 0 && item.isActive !== 0" alt=""
                   class="image-card" src="@/assets/messageService/zuanshi2.png"/>
              <span :class="{ 'first-word': index === 0 }" class="img-word">Lv{{ index + 1 }}</span>
            </div>
          </div>
        </el-card>
        <div class="pick"></div>
    ...
    <!-- 弹框部分 -->
    <PassageWayDialog :diaFormData="sendPassDialogData" :dialogShow="dialogShow" :title="passTitle"
                      @closeDialog="closeDialog"/>
    <!-- 删除动作 -->
    <el-dialog :show-close="false" :visible.sync="centerDialogVisible" center class="dia-pass" top="22vh" width="322px">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button style="padding: 10px 20px;" type="primary" @click="detelePassDialog">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default class PassagewayManage extends Vue {
  ...
  getList() {
    ...
    this.$api.getConfigPage(params).then(res => {
      if (res && res.records) {
        this.cardLiStData = res.records.map((obj: any) => ({
          choseCheck: false, ...obj,
          isActive: obj.isActive === 0 ? false : true,
          canEdit: obj.isEnable === 1 ? true : false
        }))
      }
    })
  }
