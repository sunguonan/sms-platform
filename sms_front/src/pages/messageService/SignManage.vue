<template>
  <div class="sign-message">
    ...
    <div class="list">
      <el-button style="margin-right: 26px;" type="primary" @click="addSign('add')">
        <i class="el-icon-circle-plus-outline icon-sign"></i>
        添加签名
      </el-button>
      <el-button :disabled="ids.length === 0" class="delete" @click="deleteSignIds">
    ...
    <div v-if="cardLiStData.length > 0" class="data-list">
      <el-card v-for="(item, index) of cardLiStData" :key="index" class="box-card">
        <div class="card">
          <el-checkbox v-model="item.choseCheck" class="checkbox-style"
                       @change="getSignIds(item, $event)"></el-checkbox>
          <div class="card-word">
            <span class="word-text">{{ item.code }}</span>
            <span class="card-img edit" @click="addSign('edit', item)">
              <el-tooltip class="item" content="点击修改签名模板" effect="dark" placement="top">
            ...
            <span class="card-img juli" @click="deleteSign(item)">
              <el-tooltip class="item" content="点击删除当前签名模板" effect="dark" placement="top">
                <img alt="" src="@/assets/messageService/delete.png"/>
              </el-tooltip>            
            </span>
          </div>
          <div class="name-date">
    ...
    <div v-if="pageInfo.total > 16" class="paging">
      <el-pagination :current-page="currentPage" :page-size="pageInfo.size" :total="pageInfo.total"
                     layout="total, prev, pager, next, jumper" @size-change="handleSizeChange" @current-change="handleCurrentChange">
      </el-pagination>
    </div>
    <!-- 新增和修改 -->
    <MessageListDialog :diaFormData="signFormData" :dialogShow="dialogShow" :title="DiaTile"
                       @closeDialog="closeDialog"/>
    <!-- 删除动作 -->
    <el-dialog :show-close="false" :visible.sync="centerDialogVisible" center class="dia-sign" top="22vh" width="322px">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button style="padding: 10px 20px;" type="primary" @click="deteleSignDialog">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
