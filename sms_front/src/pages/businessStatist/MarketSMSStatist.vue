<template>
  <div class="MarketSMSStatist">
    <el-breadcrumb v-if="$route.query.type === '营销'" separator="/" style="margin-bottom: 20px;margin-left: 20px">
    ...
    <m-table :columns="tableColumns" :page="pageInfo" :stripe="true" :tableData="tableData" layout="total, prev, pager, next, jumper"
      ...
      <div slot="btn" slot-scope="{row}" style="height: 40px;">
        <el-tooltip class="item" content="点击查看失败详情" effect="dark" placement="top">
          <el-button v-show="row.fail !== 0" class="btn-img" type="text" @click="goDetail(row)">
            <img alt="" src="@/assets/marketSMS/xiangqing.png"/>
          </el-button>
        </el-tooltip>
        <el-tooltip class="item" content="点击重新发送" effect="dark" placement="top">
          <el-button v-show="row.fail !== 0" class="btn-img" type="text" @click="recordsFun(row)">
            <img alt="" src="@/assets/marketSMS/xinxichakan.png"/>
          </el-button>
        </el-tooltip>
      </div>
    </m-table>
...
import {formatTodayDate, monthOneDay} from '@/utils/formValidate'
