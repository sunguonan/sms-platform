<template>
  <div class="SendRecord">
    <div class="search">
      <m-form ref="mform" :columns="formColumns" :formData="formData"></m-form>
    </div>
    <m-table :columns="tableColumns" :page="pageInfo" :stripe="true" :tableData="tableData" layout="total, prev, pager, next, jumper"
      ...
      <div slot="name" slot-scope="{row}" class="name-color" style="height: 40px;line-height: 40px;">
        <span v-if="row.status === 1" class="name-color-success">发送成功</span>
        <span v-else class="name-color">发送失败</span>
      </div>
    </m-table>
  </div>
</template>
  ...
  get formColumns() {
    return [
      ...
      },
      {
        ...
        render: () => {
          return <div>
            <el-button type="primary" icon="el-icon-search" style="margin-left: 0px;margin-right: 17px;"
