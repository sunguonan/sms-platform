<template>
  <div class="HistoryRecords">
    ...
    <m-table :columns="tableColumns" :page="pageInfo" :stripe="true" :tableData="tableData" @pageChange="pageChange">
      ...
      <div slot="btn" slot-scope="{row}">
        <el-tooltip class="item" content="点击查看" effect="dark" placement="top">
          <el-button class="btn-img" type="text"><img alt="" src="@/assets/marketSMS/xiangqing.png"/></el-button>
        </el-tooltip>
        <el-tooltip class="item" content="点击重发" effect="dark" placement="top">
          <el-button class="btn-img" type="text" @click="recordsFun(row)"><img alt=""
                                                                               src="@/assets/marketSMS/xinxichakan.png"/></el-button>
        </el-tooltip>
      </div>
    </m-table>
  </div>
  ...
  get formColumns() {
    return [
      ...
      },
      {
        ...
        render: () => {
          return <div>
            <el-button type="primary" icon="el-icon-search" style="margin-left: 34px;margin-right: 34px;"
